...
public class Abyssal_domainClient implements ClientModInitializer {
    ...
    private static UUID borderPlayerUuid;
    ...
    public void onInitializeClient() {
        ...
        EntityRendererRegistry.register(ModEntities.TERMINUS_SHIELD, TerminusShieldEntityRenderer::new);

        registerItemRenderers();

        AbyssalEffectsRenderer.init(MinecraftClient.getInstance());
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.GRAPPLE_SWING,
                (client, handler, buf, responseSender) -> {

                    boolean start = buf.readBoolean();

                    if (start && buf.readableBytes() >= 12) {
                        int x = buf.readInt();
                        int y = buf.readInt();
                        int z = buf.readInt();
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.SHADER_EFFECT, (client, handler, buf, responseSender) -> {
            String shaderName = buf.readString();

            client.execute(() -> {
                AbyssalEffectsRenderer.applyShader(shaderName);
            });
        });

        ClientPlayNetworking.registerGlobalReceiver(ModPackets.SHADER_WORLD_EFFECT, (client, handler, buf, responseSender) -> {
            double x = buf.readDouble();
            double y = buf.readDouble();
            double z = buf.readDouble();

            client.execute(() -> {
                AbyssalEffectsRenderer.triggerWorldEffect(x, y, z, 60);
            });
        });
...
private void registerItemRenderers(){
    BuiltinItemRendererRegistry.INSTANCE.register(ModItems.Terminus_Est, (stack, mode, matrices, vertexConsumers, light, overlay) -> {});
}
